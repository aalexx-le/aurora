...
**Description:** Annotate Python code with type hints according to PEP-484. When basic Python types are insufficient, import and use types from the `typing` module or external libraries like `pydantic`, `numpy`, `pandas`, etc.
...
# Complex type annotations with typing module
T = TypeVar('T')
...
def process_data(data_frame):  # Missing type
    return data_frame.mean()

# Not importing required types
def predict(array) -> numpy.ndarray:  # 'numpy' not imported
    return array * 2
```

## Rule 13: Use Logging Instead of Print

**Description:** Use the built-in `logging` module instead of print statements for outputting information during program execution. Logging provides more flexibility, control, and contextual information compared to print statements.
