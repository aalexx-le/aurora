...
import { ExportFormat, ExportPortfolioInput, ExportResult } from './dto/export.dto';
import { AssetExportData, CategoryExportData, PortfolioExportData } from './interfaces/export-data.interface';
...
export class ExportService {
  ...
  async exportPortfolio(input: ExportPortfolioInput, userId: number): Promise<ExportResult> {
    // Get portfolio data
    const portfolioData = await this.getPortfolioData(input.portfolioId, userId);
    
    // Transform data for export
    const exportData = await this.transformPortfolioData(portfolioData, input.portfolioName);
    ...
    switch (input.format) {
      case ExportFormat.PDF:
        buffer = await this.pdfExportService.generatePdf(exportData, input);
      ...
      case ExportFormat.EXCEL:
        buffer = await this.excelExportService.generateExcel(exportData);
  ...
  private async transformPortfolioData(portfolio: any, portfolioName?: string): Promise<PortfolioExportData> {
    const assetProfits = portfolio.historicalAssetProfits || [];
    
    // Calculate totals
    const totalInvestment = assetProfits.reduce((sum: number, asset: any) => sum + asset.totalCostInQuoteQty, 0);
    const currentValue = assetProfits.reduce((sum: number, asset: any) => sum + (asset.remainingQty * asset.assetInfo.lastPrice), 0);
    const totalProfit = assetProfits.reduce((sum: number, asset: any) => sum + asset.estimatedProfit, 0);
    ...
    const assets: AssetExportData[] = assetProfits.map((asset: any) => ({
      ...
      profitPercentage: asset.totalCostInQuoteQty > 0 ? (asset.estimatedProfit / asset.totalCostInQuoteQty) * 100 : 0,
    ...
    const categories: CategoryExportData[] = Array.from(categoryMap.entries()).map(([tag, data]) => ({
      tag,
      investment: data.investment,
      currentValue: data.currentValue,
      profit: data.profit,
      profitPercentage: data.investment > 0 ? (data.profit / data.investment) * 100 : 0,
      assetCount: data.assetCount,
      allocationPercentage: totalInvestment > 0 ? (data.investment / totalInvestment) * 100 : 0
    }));
  ...
  private async saveExportFile(
    ...
    const serverUrl = this.configService.get('SERVER_HOST') + ':' + this.configService.get('SERVER_PORT');
