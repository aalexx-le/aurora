...
export const useCustomerPortalSession = () => {
  const [createPortalSession, { loading, error }] = useMutation(CREATE_CUSTOMER_PORTAL_SESSION);
  ...
  const openCustomerPortal = async (subscriptionIds?: string[]) => {
    try {
      ...
      if (data?.createCustomerPortalSession?.urls?.general) {
        ...
        window.open(data.createCustomerPortalSession.urls.general.overview, '_blank');
