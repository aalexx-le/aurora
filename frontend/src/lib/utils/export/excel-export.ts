...
export async function exportToExcel(
  ...
  try {
    onProgress?.({ stage: 'Creating workbook', progress: 10, message: 'Initializing Excel workbook...' });

    const workbook = XLSX.utils.book_new();
    
    onProgress?.({ stage: 'Adding summary sheet', progress: 30, message: 'Creating summary sheet...' });
    ...
    onProgress?.({ stage: 'Adding assets sheet', progress: 50, message: 'Creating assets breakdown...' });
    
    // Add assets sheet
    addAssetsSheet(workbook, data);
    
    onProgress?.({ stage: 'Adding categories sheet', progress: 70, message: 'Creating categories summary...' });
    
    // Add categories sheet
    addCategoriesSheet(workbook, data);
    
    onProgress?.({ stage: 'Generating file', progress: 90, message: 'Finalizing Excel file...' });
...
function addSummarySheet(workbook: XLSX.WorkBook, data: ExportData): void {
  const summaryData = [
    ['XELA Finance - Portfolio Analysis Report'],
...
function addAssetsSheet(workbook: XLSX.WorkBook, data: ExportData): void {
  const headers = [
    'Symbol', 'Name', 'Balance', 'Value', 'Investment', 
    'Profit', 'Profit %', 'Category', 'Exchange', 'Price'
  ];
...
function addCategoriesSheet(workbook: XLSX.WorkBook, data: ExportData): void {
  const headers = [
    'Category', 'Value', 'Investment', 'Profit', 
    'Profit %', 'Assets', 'Portfolio %'
  ];
