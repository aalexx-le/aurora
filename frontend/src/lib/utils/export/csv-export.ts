...
export async function exportToCsv(
  ...
  try {
    onProgress?.({ stage: 'Preparing CSV data', progress: 10, message: 'Processing portfolio data...' });

    const csvContent = generateCsvContent(data, options);
    
    onProgress?.({ stage: 'Generating file', progress: 80, message: 'Creating CSV file...' });
...
function generateCsvContent(data: ExportData, options: ExportOptions): string {
  ...
  // Add header
  lines.push('XELA Finance - Portfolio Analysis Report');
  ...
  if (options.includeSummary !== false) {
    ...
    lines.push(`Total Investment,${formatCurrency(data.summary.totalInvestment)}`);
    lines.push(`Total Profit,${formatCurrency(data.summary.totalProfit)}`);
    lines.push(`Profit Percentage,${formatPercentage(data.summary.profitPercentage)}`);
    lines.push(`Number of Assets,${data.summary.assetCount}`);
    lines.push(`Last Updated,${new Date(data.summary.lastUpdated).toLocaleString()}`);
    lines.push('');
  }
  
  // Assets Breakdown
  lines.push('ASSETS BREAKDOWN');
  lines.push('Symbol,Name,Balance,Value,Investment,Profit,Profit %,Category,Exchange,Price');
