...
import { AssetExportData, CategoryExportData, ExportData, PortfolioSummary } from "./types";
...
function calculatePortfolioSummary(
  ...
  const totalValue = analyseData.reduce((sum, item) => sum + (item.remainingQty * item.price), 0);
  const totalInvestment = analyseData.reduce((sum, item) => sum + item.invest, 0);
  const totalProfit = analyseData.reduce((sum, item) => sum + item.estimatedProfit, 0);
...
function calculateCategoriesData(analyseData: AnalyseData[]): CategoryExportData[] {
  ...
  const grandTotalValue = analyseData.reduce((sum, item) => sum + (item.remainingQty * item.price), 0);
  
  // Transform to export format
  return Array.from(categoryMap.entries()).map(([name, data]) => {
    const totalProfit = data.totalValue - data.totalInvestment;
    const profitPercentage = data.totalInvestment > 0 
      ? (totalProfit / data.totalInvestment) * 100 
      : 0;
    const percentage = grandTotalValue > 0 
