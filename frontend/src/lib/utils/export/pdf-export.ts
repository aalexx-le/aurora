...
export async function exportToPdf(
  ...
  try {
    onProgress?.({ stage: 'Initializing PDF', progress: 10, message: 'Setting up PDF document...' });
    ...
    onProgress?.({ stage: 'Adding header', progress: 20, message: 'Creating document header...' });

    // Add header
    yPosition = addHeader(pdf, data, yPosition, pageWidth, margin);

    onProgress?.({ stage: 'Adding summary', progress: 40, message: 'Adding portfolio summary...' });

    // Add portfolio summary
    if (options.includeSummary !== false) {
      yPosition = addPortfolioSummary(pdf, data, yPosition, pageWidth, margin, pageHeight);
    }

    onProgress?.({ stage: 'Adding assets table', progress: 60, message: 'Creating assets breakdown...' });

    // Add assets table
    yPosition = addAssetsTable(pdf, data, yPosition, pageWidth, margin, pageHeight);

    onProgress?.({ stage: 'Adding categories', progress: 80, message: 'Adding categories summary...' });

    // Add categories summary
    yPosition = addCategoriesTable(pdf, data, yPosition, pageWidth, margin, pageHeight);

    onProgress?.({ stage: 'Finalizing PDF', progress: 90, message: 'Generating PDF file...' });
    ...
    onProgress?.({ stage: 'Downloading', progress: 100, message: 'Download starting...' });
...
function addHeader(
  ...
  pdf.text('XELA Finance - Portfolio Analysis Report', margin, yPosition);
  yPosition += 10;
...
function addAssetsTable(
  ...
  const headers = ['Symbol', 'Name', 'Value', 'Investment', 'Profit', 'Profit %'];
  ...
  data.assets.slice(0, 20).forEach((asset) => { // Limit to first 20 assets for PDF
    // Check if we need a new page
  ...
  if (data.assets.length > 20) {
    ...
    pdf.setFontSize(8);
    pdf.setTextColor(100);
    pdf.text(`... and ${data.assets.length - 20} more assets`, margin, yPosition);
    pdf.setTextColor(0);
    pdf.setFontSize(10);
  }
...
function addCategoriesTable(
  ...
  const headers = ['Category', 'Value', 'Investment', 'Profit', 'Assets', 'Portfolio %'];
...
function addFooter(
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Footer text
    pdf.text(
      'Generated by XELA Finance Management System',
      margin,
      pageHeight - 10
    );
  }
