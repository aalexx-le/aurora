...
export class ExportService {
  static async exportPortfolio(
    ...
    try {
      onProgress?.({ stage: 'Starting export', progress: 0, message: 'Preparing data...' });
      ...
      onProgress?.({ stage: 'Data prepared', progress: 5, message: 'Starting export generation...' });
      ...
      return {
        success: true,
        fileName: generateFileName(exportData.summary.portfolioName, format)
      };
    } catch (error) {
  ...
  static validateExportData(analyseData: AnalyseData[]): { isValid: boolean; error?: string } {
    ...
    const hasValidData = analyseData.every(item => 
      item.name && 
